...
import { KT_SECTIONS } from "@/modules/documents/kt-data";
import { KTSectionContent } from "./kt-section-content";
import { KTSearchResults } from "./kt-search-results";
...
export default function KTGuidePage() {
  const searchParams = useSearchParams();
  const paramSection = searchParams.get("section");
  const paramSub = searchParams.get("sub");

  const [activeSection, setActiveSection] = useState(
    () => (paramSection && KT_SECTIONS.find((s) => s.id === paramSection) ? paramSection : KT_SECTIONS[0].id),
  );
  ...
  useEffect(() => {
    if (paramSection && KT_SECTIONS.find((s) => s.id === paramSection)) {
  ...
  const currentSection = useMemo(
    () => KT_SECTIONS.find((s) => s.id === activeSection) ?? KT_SECTIONS[0],
    [activeSection],
  );
